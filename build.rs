...
fn write_sections_to_files(data: Vec<parse_qapi::Section>) {
    ...
    let mut events = OpenOptions::new()
        ...
        .open(events_src)
        .unwrap();
    let mut structs = OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(structs_src)
        .unwrap();
    let mut enums = OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(enums_src)
        .unwrap();
    ...
    commands
        .write("use rustc_serialize::json as rustc_json;\n".as_bytes())
        .unwrap();
    commands
        .write("use rustc_serialize::Decodable as rustc_decodable;\n".as_bytes())
        .unwrap();
...
pub fn main() {
    ...
    let output = PathBuf::from("src/output.rs");
    let mut events = OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(output)
    ...
    for source in json_sources {
        let result = match parse_qapi::generate_rust_definitions(source) {
            Ok(defs) => {
                println!("Definitions: {}", defs);

                events.write(defs.as_bytes()).unwrap();
                events.sync_all().unwrap();
            }
            Err(e) => debug!("Failed to generate from {}: {}", source, e),
        };
    }
